					...
					<table id="table_historique">
						...
						{foreach from=$table_factures.$i item=valeur}
							...
							<tr>
								...
								<td>{$valeur.type}</td>
								<td>{$valeur.date_obtention|FD_JJMMAAHHMMSS}</td>
							</tr>
			...
			{elseif $smarty.get.onglet eq 'objet'}
				{if isset($fichier_film)}
					...
					<video controls width="600">
						<source src="film/{$fichier_film.fichier}" type="video/ogg" />
					</video>
				{elseif isset($fichier_livre)}
					<h2>{$fichier_livre.designation}</h2>
					<iframe src="book/{$fichier_livre.fichier}" width="600" height="500" align="middle"></iframe>
				{else}
